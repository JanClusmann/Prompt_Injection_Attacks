...
plot_2c <- ggplot(data_2c, aes(x = `Position of adversarial prompt`, y = mean, fill = Model)) +
  ...
  # geom_jitter(data = data_selection, aes(y = Harmfulness_Mean, color = Model), shape = 21, fill = NA, size = 2, stroke = 0.5,
...
# Print the results
print("Adjusted p-values for Organ detection Modality comparisons:")
...
export_stats_to_excel(
  ...
  p_values = results_modality_df  # Empty list for p_values
)
...
final_p_values_list <- list(
  "Modality Comparisons" = results_modality_df,
  "Lesion Miss Rate by Prompt Type" = harmfulness_by_prompt,
  "Attack Success Rate" = attack_success_rate,
